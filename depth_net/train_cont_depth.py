...
class Dfd(object):
    def __init__(self, net=None, config=DfdConfig(), device=None, train=True):
        self.config = config
        if train:
    ...
    def prepare_for_net(self, np_arr, device=None):
        ...
        np_arr = np_arr.to(device) if device is not None else np_arr.to(self.device)
