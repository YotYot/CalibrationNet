...
class DfdConfig(object):
    def __init__(self, batch_size=64, mode='segmentation', image_size='small',
                 resume_path=os.path.join(RES_OUT, 'best_model.pt'), start_epoch=0, end_epoch=50,
                 model_state_name='best_model.pt', num_classes=15, lr=0.01, get_reduced_dataset=False, dataset=None, ordinal=False):
...
class Dfd(object):
    def __init__(self, net=None, ordinal_net=None, config=DfdConfig(), device=None):
        ...
        self.ordinal_net = ordinal_net if ordinal_net else ResNet18(mode=config.mode, image_size=config.image_size,
                                                                    num_classes=self.config.num_classes)
        ...
        self.colors = (
            (0, 0, 0), (255, 255, 255), (255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0), (0, 255, 255),
            (255, 0, 255),
        ...
        self.conf_matrix_lt = np.zeros((config.num_classes, config.num_classes), dtype=np.uint32)
    ...
    def get_plus_minus_correct(self, a):
        return np.sum(np.diag(a)) + np.sum(np.diag(a[1:,:])) + np.sum(np.diag(a[:,1:]))
    ...
    def resume(self, resume_path=None, ordinal=False):
        ...
        if resume_path:
            if os.path.isfile(resume_path):
                ...
                if ordinal:
                    self.ordinal_net.load_state_dict(checkpoint['state_dict'])
                else:
    ...
    def show_imgs_and_seg(self, image_file=None, label_file=None, patch_size=32):
        ...
        with torch.no_grad():
            if not image_file:
                ...
                img_predictions_image = np.zeros((img_predictions.shape[0], img_predictions.shape[1], 3),
                                                 dtype=np.uint8)
                ...
                for i in range(end_x):
                    for j in range(end_y):
                        img_predictions[i * patch_size:(i + 1) * patch_size, j * patch_size:(j + 1) * patch_size] = (
                        img_patches_predictions[i, j]).numpy()
    ...
    def evaluate_model(self, partial=False, threshold=0.5, net=None, get_ext_class_from_ordinal=False):
        if not net:
            net = self.net;
        net = net.eval()
        ...
        correct_plus_minus_lt = 0
        self.conf_matrix = np.zeros_like(self.conf_matrix)
        self.conf_matrix_lt = np.zeros_like(self.conf_matrix_lt)
        with torch.no_grad():
            for data in self.testloader:
                images, labels = data
                ...
                if self.config.mode == 'segmentation':
                    ...
                    if (get_ext_class_from_ordinal and predicted in [0,1,13, 14]):
                    ...
                    if self.config.ordinal:
                        ...
                        predicted_lt = self.first_index_lt(outputs, threshold)
                        correct_lt += (predicted_lt == labels).sum().item()
                        correct_exact_lt = (predicted_lt == labels)
                        correct_plus_lt = (predicted_lt == labels + 1).item()
                        correct_minus_lt = (predicted_lt == labels - 1).item()
                        correct_plus_minus_lt += (correct_exact_lt | correct_plus_lt | correct_minus_lt).sum().item()
                        self.conf_matrix_lt[labels.item()][predicted_lt.item()] += 1
                        acc_lt = 100 * correct_lt / total
                        acc_plus_minus_lt = 100 * correct_plus_minus_lt / total
                        print('Accuracy on val images lt: %d %%' % (acc_lt))
                        print('Accuracy on label or -/+ 1 of the label lt: %d %%' % (acc_plus_minus_lt))
                        predicted = predicted_lt
                    correct += (predicted == labels).sum().item()
                    correct_exact = (predicted == labels)
                ...
                if self.config.ordinal:
                    ...
                    for ii, label in enumerate(labels):
                        for j in range(label):
                            ord_labels[ii, j] = 1
                    labels = torch.from_numpy(ord_labels)
...
if __name__ == '__main__':
    ...
    config = DfdConfig(image_size='small', batch_size=32, mode='classification', lr=0.001, get_reduced_dataset=False,
                       num_classes=15, dataset='ours', ordinal=False)
    ...
    dfd.resume(resume_path=os.path.join(RES_OUT,'best_model_ordinal_classification.pt'), ordinal=True)
    dfd.resume(resume_path=os.path.join(RES_OUT,'best_model_90pm1.pt'))
    dfd.evaluate_model(partial=True, get_ext_class_from_ordinal=True)
    dfd.train()
    dfd.plot_metrics()
    dfd.show_imgs_and_seg(image_file='/home/yotamg/data/jpg_images/City_0212_rot1.jpg',
                          label_file='/home/yotamg/data/depth_pngs/City_0212_rot1.png', patch_size=32)
