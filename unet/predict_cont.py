...
def predict_img(net,
    ...
    # img = hwc_to_chw(img)
    img = torch.unsqueeze(full_img,0)
...
def predict_full_img(net, img, device):
    ...
    img = img[0]
    mask = torch.zeros((img.shape[1], img.shape[2])).to(device)
    x0 = 0
    for i in range((img.shape[2] // (patch_w - margin)) + 1):
        y0 = 0
        y_part_margin = 0
        for j in range((img.shape[1] // (patch_h - margin)) + 1):
            x1 = min(img.shape[2], x0+patch_w)
            y1 = min(img.shape[1], y0+patch_h)
            # img_part = img.crop((x0,y0,x1,y1))
            img_part = img[:, y0:y1,x0:x1]
            mask_part = predict_img(net=net,
                                    full_img=img_part,
                                    device=device)
            mask = implant_mask_part(img, mask, mask_part[0], x0, x1, x_part_margin, y0, y1, y_part_margin)
...
def implant_mask_part(img, mask, mask_part, x0, x1, x_part_margin, y0, y1, y_part_margin):
    if y1 == img.shape[1] and x1 == img.shape[2]:
